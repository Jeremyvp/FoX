...
auto fox::game::run() -> void {
    ...
    m_component_system.add_component(components::idler{.base_duration = 3'000ms});
...
auto fox::game::update() -> void {
    ...
    m_component_system.update_components<components::idler>(delta, 1.f, 0ms);
}
...
auto fox::game::draw() -> void {
    ...
    for (const auto& idler : m_component_system.get_components<components::idler>())
